...
class ALTLB_IG(MaxValueBase):
    r"""The acquisition function for General-purpose Information-Based
    Bayesian Optimisation (GIBBON) with an alternative lower bounds. This is comes from assuming the batch form
    where each batch point is taken at the same location.
    ...
    def forward(self, X: Tensor) -> Tensor:
        ...
        # prepare squared correlation between current and target fidelity
        rho_sq = sigma_2_f / (sigma_2_eps + sigma_2_f)

        # batch_shape x 1
        check_no_nans(rho_sq)

        #Calculate first and second exploration terms
